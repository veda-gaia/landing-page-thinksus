...
export class EIndustryComponent {
      ...
      .subscribe({
        next: (data) => {
          if (data.section !== 'Industry') {
            this.toastr.error(
              'Esta avaliação não corresponde ao setor da sua empresa',
              'Erro',
              { progressBar: true }
            );
              ...
              answers: this.formArray.controls.map((control, index) => {
                return {
                  ...
                  documentsPath: this.questionaryData[index].documentNeeded
                    ? data
                        .filter(
                          (item) => item.name == this.questionaryData[index].id
                        )
                        .map((item) => item.url)
                    : null,
                };
  ...
  checkDoesntApply() {
    this.undefinedAnswers = this.formArray.controls.reduce(
      (acc: number, cur: FormControl) => {
